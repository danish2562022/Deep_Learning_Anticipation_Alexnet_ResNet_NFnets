...
def load_ops(subfolders,opts):
	...
	for path in subfolders:
		#print("path in subfolder",path)

		x= 'C:/Users/Deep_Learning/Desktop/Dataset_Cholec/ins_ant-master'+path[2:]
		#print(x)
		...
		for ID in os.listdir():
			op_path = os.path.join(os.getcwd(), ID)
			#print("op_path",op_path)
			if os.path.isdir(op_path):
				anno_file = opts.annotation_folder + "video" + ID + "-tool.txt"
				dataset = Cholec80Anticipation(op_path, anno_file, opts.width, opts.height, opts.horizon)
				dataloader = data.DataLoader(dataset, batch_size=opts.batch_size, shuffle=False, num_workers=4)
...
class Cholec80Anticipation(data.Dataset):
	def __init__(self, image_path, annotation_path, width, height, horizon=5):
		...
		annotation_path='C:/Users/Deep_Learning/Desktop/Dataset_Cholec/ins_ant-master'+annotation_path[2:]
