...
class BayesFeatureAlexNet(nn.Module):
	def __init__(self, num_features = 4096):
		super(BayesFeatureAlexNet, self).__init__()
	...
	def init_dropout_mask(self, prob=.2):
		return (nn.Dropout(p=prob)(torch.ones(64,53,95)).cuda(),
	...
	def set_mode(self, mode='VARIATIONAL'):
		if mode in ['VARIATIONAL', 'DETERMINISTIC']:
			...
			print('Mode unchanged since {} is not a valid mode. Possible modes are VARIATIONAL and DETERMINISTIC'.format(mode))
...
class BayesLSTM(nn.Module):
	...
	def init_dropout_mask(self, prob=.2, batch_size=1):
		return (nn.Dropout(p=prob)(torch.ones(batch_size,self.input_size)).cuda(),
...
class BayesLSTMAlexNet(nn.Module):
	def __init__(self, num_classes, lstm_size=512, lstm_input_size=4096, pretrain=None):
		super(BayesLSTMAlexNet, self).__init__()

		self.featureNet = BayesFeatureAlexNet(num_features=lstm_input_size)
